...
__weak void MCP_ReceivedFrame(MCP_Handle_t *pHandle, uint8_t Code, uint8_t *buffer, uint8_t Size)
{
  ...
  case MC_PROTOCOL_CODE_GET_MP_INFO:
    ...
    case MC_PROTOCOL_CODE_ES_CONTROL:
    {
        ...
        switch(bSysID)
        {
          case MC_Protocol_ES_Throttle:
          {
              bNoError = true;
              pHandle -> fFcpSend(pHandle->pFCP, ACK_NOERROR,&THROTTLE,1);
          ...
          {
             bNoError = true;
             pHandle -> fFcpSend(pHandle->pFCP, ACK_NOERROR,&BRAKE,1);
          ...
          break;
   	      ...
   	      case MC_Protocol_ES_Mode_Config:
   	      {
              bNoError = true;
    ...
    case MC_PROTOCOL_TORQUE_RAMP_CONFIG:
      {
      	...
      	pHandle -> fFcpSend(pHandle->pFCP, ACK_NOERROR, &debug_config_success,1);
      }
      break;

    case MC_PROTOCOL_TORQUE_DYNAMIC_CONFIG:
      {
      	/*Decode the input dummy data, pass them to the api or UART2 fcpSend*/
